...
Alur:
    ...
    4. Random Forest classify hasil prediksi → kelas awan + risiko banjir
...
# Output
OUTPUT_DIR   = Path("./RF_forecast_output")
...
def rf_forecast(series: list[np.ndarray],
                n_steps_ahead: int,
                n_estimators: int = 20) -> np.ndarray:
    """
    Forecasting CTT menggunakan Random Forest Regression.

    Konsep:
        - Setiap window historis digunakan sebagai history untuk prediksi
        - Target = CTT pada t + horizon
        - Model dilatih per piksel menggunakan indeks waktu sebagai fitur
    """
    n = len(series)
    ...
    for i in range(G):
        for j in range(G):
            y = stack[:, i, j]
            rf_reg.fit(t_values, y)
            forecast[i, j] = rf_reg.predict(t_pred)[0]
...
def run_forecast_experiment(time_series: list[tuple[datetime, np.ndarray]],
    ...
    for i in range(window, n - horizon):
        ...
        # Prediksi
        pred_ctt = rf_forecast(input_frames, n_steps_ahead=horizon)
        ...
        # Gunakan model Random Forest yang sudah dilatih untuk mengklasifikasikan CTT
        X_pred = pred_ctt.flatten().reshape(-1, 1)
        X_pred_sc = scaler.transform(X_pred)
        pred_class_rf = rf.predict(X_pred_sc).reshape(pred_ctt.shape)

        acc_ctt_threshold = float(np.mean(actual_class == pred_class_threshold) * 100)
        acc_ctt_rf = float(np.mean(actual_class == pred_class_rf) * 100)
        acc_list_ctt.append(acc_ctt_rf)
...
def build_rf_model() -> tuple[RandomForestClassifier, StandardScaler]:
    """Load atau build model Random Forest."""
    model_path  = MODEL_DIR / "rf_model.pkl"
    scaler_path = MODEL_DIR / "scaler_rf.pkl"
    ...
    if model_path.exists() and scaler_path.exists():
        try:
            ...
            if not isinstance(rf, RandomForestClassifier):
                raise ValueError("Saved model is not a RandomForestClassifier")
            if getattr(rf, "n_features_in_", None) not in (None, 1):
                raise ValueError(
                    f"Saved RF model expects {rf.n_features_in_} features"
                )
            ...
            print(f"  Warning: saved RF model invalid ({e}). Rebuilding model.")
...
def plot_single_frame(result: dict,
    ...
    fig1.text(0.5, 0.91,
              ...
              f"Akurasi RF: {result['acc_ctt_rf']:.1f}%",
              ha="center", va="top",
...
def plot_comparison_chart(all_metrics: dict, out_path: Path) -> None:
    ...
    for lbl, color, m in zip(labels, colors, metrics):
        if m and m.get("results"):
            accs = [r.get("acc_ctt_rf", r.get("acc_ctt_threshold", 0)) for r in m["results"]]
...
def main():
    ...
    print("  Bandung & Sekitarnya | Himawari-9 + Random Forest + Trend Forecasting")
    print("="*65)
    ...
    for interval_label, interval_min in INTERVALS.items():
        ...
        metrics = run_forecast_experiment(ts_resampled, interval_min, rf, scaler)
